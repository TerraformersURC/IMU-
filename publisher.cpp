...
public:
    ...
    {   
        ...
        shm_data = (shared_data *)shmat(shmid, nullptr, 0);
        if (shm_data == (void *)-1) {
            RCLCPP_ERROR(this->get_logger(), "Failed to attach to shared memory");
            rclcpp::shutdown();
            return;
        }
...
private:
    void publish_data()
    {
        ...
        // RCLCPP_INFO(this->get_logger(), "Acc: %.3f %.3f %.3f", shm_data->fAcc[0], shm_data->fAcc[1], shm_data->fAcc[2]);
